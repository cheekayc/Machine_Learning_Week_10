...
1. What are some limitations of these data that may impact your ability to draw inferences from your analysis?



2. Sketch out an analytic pipeline to address your task. Similar to Assignment 7, your pipeline should include the following components:
   * Data Preparation: same process as we have done for prediction.
   * Partitioning/Resampling: depends on our research question. In feature selections, we assume that what's important for prediction is also important for causality. 
     In machine learning, the algorithms we used build the "best" model and show us the importance of each variable, helping us to identify which variables are important for
     predictions, which in turn these are also the variables important for hypothesis testing. The assumptions we made rely on the validity of the prediction. 
     In short, partitioning is still important in unsupervised learning.
